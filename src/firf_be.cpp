...
firf_be::firf_be(double samplerate, double error_max, double freq_bw_min,
	double win_pow, double delay_frac)
{
	auto temp_firf = std::make_unique<firf_be>();
...
void firf_be::Set_Parameters(double freq_center, double freq_bw,
	double atten)
{
...
void firf_be::Configure(double samplerate, double error_max, double freq_bw_min,
...
std::vector<double> firf_be::Filter(std::vector<double> const& signal)
{
	if (!Valid_Firf_Base())
	{
		throw config_error("Invalid filter configuration");
...
bool firf_be::Valid_Freq_Bw_Parameter(double freq_center,
...
bool firf_be::Valid_Fshift_Parameter(double freq_shift,
	double samplerate) const
{
...
bool firf_be::Valid_atten_Parameter(double atten) const
{
